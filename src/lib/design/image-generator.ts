...
Maintain the exact quality and details of the original subject.`,
    ...
    console.warn(
      "Background removal did not return edited image, using original",
    );
...
export async function uploadToBrandAssets(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to upload image to brand assets: ${response.status}`,
    );
  }
  ...
  return (
    result.url ||
    result.assetUrl ||
    `https://cdn.bannerbros.net/brand/${brandId}/${finalFilename}`
  );
}
...
export async function generateAndUploadImage(
  ...
  try {
    ...
    console.warn(
      "[ImageGenerator] Upload failed, using data URL:",
      uploadError,
    );
    ...
    console.log(
      "[ImageGenerator] Using data URL (length:",
      imageUrl.length,
      ")",
    );
  }
